#!/bin/bash
CONFIGURATION='{"link":{"host":"sensor.cloud.tenable.com","port":443,"key":"'"$LINKING_KEY"'","name":"'"$AGENT_NAME"'","groups":["'"$AGENT_GROUP"'"]}}'
SERVER='sensor.cloud.tenable.com:443'
FCVER=38
...
centos9=$(echo "$release" | grep -c "centos:9")
...
function populate_oracle9 {
    ...
    if [[ $aarch64 -gt 0 ]]
    then
        file=NessusAgent-el9.aarch64.rpm
    else
        file=NessusAgent-el9.x86_64.rpm
    fi
...
function populate_oracle8 {
    ...
    if [[ $aarch64 -gt 0 ]]
    then
        file=NessusAgent-el8.aarch64.rpm
    else
        file=NessusAgent-el8.x86_64.rpm
    fi
...
function populate_oracle7 {
    ...
    if [[ $is64bit -gt 0 ]]
    then
        file=NessusAgent-el7.x86_64.rpm
    fi
...
function populate_ubuntu {
    ...
    then
        file=NessusAgent-ubuntu1604_amd64.deb
    fi
...
response=$(curl -H "X-Key: '"$LINKING_KEY"'" -s -w "%{http_code}" https://$SERVER/install/agent/installer/$file -o $file)
